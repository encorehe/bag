...
var bag={
    _init:function () {
        var powers = temD[1].power, //权限
            _con = $classAlbums; //班级相册配置信息
        common.powerFlag(powers);

        var items = [
            { title: '相册标题', name: 'title', width:120, align: 'center' },
            ...
            { title: '创建时间', name: 'insertDate', width: 60, align: 'center' },
            {
                ...
                renderer: function(val, row) {
                    ...
                    imgBtns = "<a class='iconfont J-imgs' href='javascript:void(0);' title='查看照片'>&#xe647;</a>",
                    btnsHtml = ckBtns;
                    if(row.classId != 0){ 
                        if (row.isUse == 0) {
                ...
                if(ajaxDatas.success){
                    ...
                    /*laypage({
                        ...
                        jump: function(obj, first){
                            var cab = function () {
                                if(ajaxDatas.list.length>0){
                                    $('.no-empty').hide();
                                }else{
                                    $('.no-empty').show();
                                }
                                var _html = template('tplPhotos',{list:ajaxDatas.list,"time":timestamp});
                                debugger;
                                ele.html(_html);
                            };
                            common.ajaxSubmitV(url,datas,cab);
                        }
                    });*/
                            ...
                            classAlbumId=datasCur.id;
                             ...
                             successBack=function(){
                                var callback = function(){
                                    ...
                                    common.upPhoto($('#fileName'),callback);
                             };
        ...
        mmg.unbind('cellSelected').on('cellSelected',function(e,item,rowIndex,colIndex){
            ...
            if ($(e.target).is('.J-imgs')) {
               // layer.closeAll();
                ...
                common.layerBox('相册',area,content,successBack);
            }
            ...
            if ($(e.target).is('.J-edit')) {
                ...
                zhuangtaiTag="bianji";
                mmg.select(rowIndex);
                mainCont.editDatas(item);
            }
            if($(e.target).is('.J-tingyong')){
                e.stopPropagation(); //停用
                mmg.select(rowIndex);
                mainCont.delDataF(0, datasCur,delUrl);
            }
            if($(e.target).is('.J-qiyong')){
                e.stopPropagation(); //启用
                mmg.select(rowIndex);
                mainCont.delDataF(1, datasCur,qiyongUrl);
            }
        })
    },
    formEmpty:function () {
