...
var bag={
    _init:function () {
        var powers = temD[1].power, //权限
            _con = $productOrders; //商城管理-订单配置信息
        common.powerFlag(powers);

        var items = [
            { title:'订单号', name:'orderCode' ,width:120, align:'center'},
            ...
            {title:'订单商品详细',name:'orderDetalList', align: 'left',width:190,renderer:function(val,row){
                var _html="";
                for(var i=0;i<val.length;i++){
                    _html+='<span class="spandisc">商品:'+val[i].productName+'<i>数量：'+val[i].number+'</i>单价:'+val[i].price+'</span>'
                }
                return _html;
            }},
                ...
                var title="订单支付",area=['415px', '235px'],content=template('tplPay',item),
                    successBack=function () {
                       ...
                       $('.layui-layer-btn').remove();
                        var data={"out_trade_no":datasCur.orderCode};
                        elePay.on('click',function () {
                            var th=$(this);
                            th.addClass('disabled');
                            window.open('./tpl/pays.html?out_trade_no='+datasCur.orderCode);
                        });
                    };
