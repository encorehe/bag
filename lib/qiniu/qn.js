///<jscompress sourcefile="qiniu.js" />
...
function QiniuJsSDK() {
    ...
    this.avthumb = function (op, key) {
        var xhr = this.createAjax();
        xhr.open('POST', 'http://api.qiniu.com/pfop/', false);
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                info = that.parseJSON(xhr.responseText);
            }
        };
        var path="bucket=bs-video&key="+key+"&force=1&fops=avthumb%2fmp4%2fwmImage%2f" + this.URLSafeBase64Encode(op.wmImage) + "|saveas%2f" + this.URLSafeBase64Encode('bs-video:' + key);
        xhr.setRequestHeader('Content-type','application/x-www-form-urlencoded');
        xhr.setRequestHeader('Authorization','QBox '+op.accessToken);
        xhr.send(path);
        return info;
        /*var videoUrl = 'avthumb/mp4';
        var wmImage = op.wmImage || '';
        if (!wmImage) {
            return false;
        }
        videoUrl += wmImage ? '|wmImage/' + this.URLSafeBase64Encode(wmImage) : '';
        videoUrl += '|saveas/' + this.URLSafeBase64Encode('bs-video:' + key);
        if (key) {
            videoUrl = this.getUrl(key) + '?' + videoUrl;
        }
        return videoUrl;*/
...
///<jscompress sourcefile="qiniu.uploader.js" />
...
function errText(errcode) {
    var returnerrText='';
        switch (errcode) {
            case 400:
                returnerrText = "请求报文格式错误。";
                break;
            case 401:
                returnerrText = "客户端认证授权失败。请重试或提交反馈。";
                break;
            case 405:
                returnerrText = "客户端请求错误。请重试或提交反馈。";
                break;
            case 579:
                returnerrText = "资源上传成功，但回调失败。";
                break;
            case 599:
                returnerrText = "网络连接异常。请重试或提交反馈。";
                break;
            case 614:
                returnerrText = "文件已存在。";
                break;
            case 631:
                returnerrText = "指定空间不存在。";
                break;
            case 701:
                returnerrText = "上传数据块校验出错。请重试或提交反馈。";
                break;
            default:
                returnerrText = "未知错误。";
                break;
        }
        return returnerrText;
...
(function Qiniu() {
    ...
    var Qiniu_BLKSize = 10 * 1024 * 1024;
                        ...
                        if (blkRet["crc32"] != crc) {
                            if (--retry == 0) {
                                if (events["putFailure"]) {
                                    ...
                                    fireEvent("putFailure")(errText(xhr.status));
                                }
                            } else {
                                //console.log("mkblk failded,retyr = ", retry);
                                Qiniu_mkblk(file, blkIdex, blksize, blkCnt, r, retry);
